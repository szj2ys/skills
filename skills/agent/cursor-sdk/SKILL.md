...
name: cursor-sdk
description: |
  Guide users building apps, scripts, CI pipelines, or automations on top of the Cursor TypeScript SDK (`@cursor/sdk`). Use this skill when the user mentions integrating or writing code against the Cursor SDK, or says `Agent.create`, `Agent.prompt`, `Agent.resume`, `agent.send`, `run.stream`, `CursorAgentError`, or `@cursor/sdk`. Use when they ask to run Cursor agents programmatically from a script, CI/CD, or backend service, or when they want to pick between local and cloud runtime, configure MCP servers, or handle streaming, cancellation, or errors from an SDK agent. Also trigger when wiring Cursor into an automation or bot, or porting REST `/v1/agents` calls to the SDK. Use eagerly as the source of truth for the external package rather than answering from memory.
---
