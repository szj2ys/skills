...
You are tasked with creating git commit for the changes.
...
3. **Execute the commit (single batch)**
   - Use `git add` to stage relevant files
   - Commit **all staged changes at once** (do not split into multiple commits unless explicitly requested)
   - If the current branch already has a corresponding remote branch, push the commit to remote. If the branch exists only locally and is not yet associated with any remote branch, ask the user whether they want to create a remote branch for it.
